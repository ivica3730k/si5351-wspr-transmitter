...
public:
    ...
    {
        const char* files[] = {
            FILE_CALLSIGN, FILE_LOCATOR,
            FILE_20_FREQ, FILE_20_CORR, FILE_20_DRIVE, FILE_20_POWER,
            FILE_15_FREQ, FILE_15_CORR, FILE_15_DRIVE, FILE_15_POWER,
            FILE_10_FREQ, FILE_10_CORR, FILE_10_DRIVE, FILE_10_POWER
        };
    ...
    TxParameters get_tx_params_20m() override
    {
        return TxParameters(
            ...
            static_cast<TxParameters::DriveStrength>(read_uint8(FILE_20_DRIVE, static_cast<uint8_t>(TxParameters::DriveStrength::MEDIUM_POWER))),
    ...
    TxParameters get_tx_params_15m() override
    {
        return TxParameters(
            ...
            static_cast<TxParameters::DriveStrength>(read_uint8(FILE_15_DRIVE, static_cast<uint8_t>(TxParameters::DriveStrength::MEDIUM_POWER))),
    ...
    TxParameters get_tx_params_10m() override
    {
        return TxParameters(
            ...
            static_cast<TxParameters::DriveStrength>(read_uint8(FILE_10_DRIVE, static_cast<uint8_t>(TxParameters::DriveStrength::MEDIUM_POWER))),
...
private:
    ...
    {
        if (!LittleFS.exists(path)) return String(default_val);
        File file = LittleFS.open(path, "r");
        if (file) {
            String value = file.readStringUntil('\n');
            file.close();
            value.trim();
            Serial.println("TxParametersControllerLittleFs: Successfully read string from LittleFS for file " + String(path) + ": " + value);
            return value;
        }
        Serial.println("TxParametersControllerLittleFs: Failed to read string from LittleFS for file " + String(path) + ", returning default value of " + String(default_val));
    ...
    {
        if (!LittleFS.exists(path)) return default_val;
        File file = LittleFS.open(path, "r");
        if (file) {
            uint8_t val;
            if (file.read(&val, 1) == 1) {
                file.close();
                Serial.println("TxParametersControllerLittleFs: Successfully read uint8_t from LittleFS for file " + String(path) + ": " + String(val));
                return val;
            }
            file.close();
        }
        Serial.println("TxParametersControllerLittleFs: Failed to read uint8_t from LittleFS for file " + String(path) + ", returning default value of " + String(default_val));
        return default_val;
    }
    ...
    int32_t read_int32(const char *path, int32_t default_val)
    {
        ...
        if (file) {
            ...
            if (file.read((uint8_t*)&val, sizeof(val)) == sizeof(val)) {
                file.close();
                Serial.println("TxParametersControllerLittleFs: Successfully read int32_t from LittleFS for file " + String(path) + ": " + String(val));
                return val;
            }
            file.close();
        }
        Serial.println("TxParametersControllerLittleFs: Failed to read int32_t from LittleFS for file " + String(path) + ", returning default value of " + String(default_val));
        return default_val;
    }
    ...
    uint64_t read_uint64(const char *path, uint64_t default_val)
    {
        ...
        if (file) {
            ...
            if (file.read((uint8_t*)&val, sizeof(val)) == sizeof(val)) {
                file.close();
                Serial.println("TxParametersControllerLittleFs: Successfully read uint64_t from LittleFS for file " + String(path) + ": " + String(val));
                return val;
            }
            file.close();
        }
        Serial.println("TxParametersControllerLittleFs: Failed to read uint64_t from LittleFS for file " + String(path) + ", returning default value of " + String(default_val));
        return default_val;
    }
};
